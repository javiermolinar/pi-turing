...
export const scholarlyProviderSchema = z.enum(["openalex", "crossref"]);
...
export const discoveryWorkSchema = z.object({
  ...
  date: z.object({ value: z.string().max(10), kind: z.enum(["publication", "registration"]), precision: z.enum(["year", "month", "day"]) }).optional(),
...
export const discoveryBatchSchema = z.object({
  ...
  coverage: z.array(z.object({ provider: scholarlyProviderSchema, status: z.enum(["ok", "partial", "failed"]), count: z.number().int().nonnegative(), skipped: z.number().int().nonnegative(), cached: z.boolean(), error: z.string().max(300).optional() })).max(10),
...
export interface ScholarlyAdapter {
  ...
  endpoint(query: string, contact?: string): URL;
