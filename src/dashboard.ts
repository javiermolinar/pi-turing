...
export function renderShell(state?: RunState, live = false, runnerLive = false): string {
  ...
  <main id="main" ${state ? "" : "hidden"}>${state ? renderMain(state, live && runnerLive) : ""}</main><script${live ? ' src="app.js"' : ""}>${live ? "" : dashboardScript}</script></body></html>`;
}
