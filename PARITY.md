...
| Legacy retraction refresh | Temporary Python OpenAlex operation for backend public-source notes, gated by the saved approval list; local/scoped snapshot coverage remains explicitly unknown | `tests/approval_test.py`; Semantic Scholar fallback disabled before cache or transport; absent OpenAlex approval blocks the gate explicitly; TypeScript replacement pending |
...
| Additional scoped capabilities | Approved, hash-pinned read-only adapter registry plus explicit-file-collection reader | `tests/capabilities.test.ts`, runner tests; immutable document captures, origin/version/privacy, completeness and full-read distinctions, malicious procedure isolation, fixed scopes, cancellation and revoked/changed binding refusal. No production knowledge-base connector is enabled; each needs its own contract tests and approval |
| Contradiction/depth/reconciliation/digest | Not implemented | M4 scheduling, evidence fixtures and evaluations pending |
| Corpus critique / independence analysis | Host-side relationship helpers; no scheduled corpus critic | `tests/audit.test.ts`: identical and near-copied articles form one conservative cluster; unmatched sources remain unverified. Shared-primary tracing, persisted stage results and live evaluations pending |
| Claim-support audit and re-audit | Tested proof/coverage/finding contracts and journaled correction loop; not yet in the runner pipeline | `tests/audit.test.ts`, `tests/audit_test.py`: raw-text quote checks, stale hashes, missing reads, revoked local evidence, false numerical direction, fabricated quotes and missing bindings. Async SDK validation is awaited. `tests/audit-loop.test.ts` covers batch checkpoints, pause around patch publication, cost exhaustion, stale evidence, correction limits and complete re-audit. Prose-block assessment is not sentence-level entailment or proof of truth; production runner wiring and comparative evaluations pending |
