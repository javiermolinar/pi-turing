...
import { portableMarkdown, publicUrl } from "./export.ts";
...
export function renderShell(state?: RunState, live = false, runnerLive = false): string {
  ...
  const e = escapeHtml;
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="${e(csp)}"><meta name="referrer" content="no-referrer"><title>Hyperresearch${state ? ` · ${e(state.tag)}` : " · Investigations"}</title>${live ? '<link rel="stylesheet" href="style.css">' : `<style>${dashboardCss}</style>`}</head><body data-live="${live}" data-run="${e(state?.tag ?? "")}"><nav class="topbar" aria-label="Dashboard"><span class="brand"><b>◈</b> Hyperresearch</span><span id="connection" class="connection" role="status">${live ? "Connecting…" : "Offline snapshot"}</span><div class="controls">${live ? '<button id="inventory-button">Investigations</button>' : ""}<input id="source-filter" type="search" placeholder="Filter sources…" aria-label="Filter sources"><button id="refresh">Refresh</button><button id="export-markdown" ${state?.report === undefined ? "disabled" : ""}>Export Markdown</button><button id="export-html" ${!live ? "hidden" : ""}>Export HTML</button></div></nav>
  ...
  <main id="main" ${state ? "" : "hidden"}>${state ? renderMain(state, live && runnerLive) : ""}</main>${markdown !== undefined ? `<template id="markdown-data">${Buffer.from(markdown).toString("base64")}</template>` : ""}<script${live ? ' src="app.js"' : ""}>${live ? "" : dashboardScript}</script></body></html>`;
}
