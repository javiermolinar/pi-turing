---
name: turing
description: Run Turing's light research pipeline with private working state and a live dashboard. Use when the user explicitly requests Turing research or asks to start this package's research pipeline. Formerly pi-hyperresearch.
---
...
Configuration is `.pi/turing.json`; its accepted fields are defined in `src/types.ts`. The legacy `.pi/hyperresearch.json` is used only when the new file is absent. `/hyperresearch` remains a command alias. Existing storage stays at `~/.pi/hyperresearch`; do not move or rewrite runs for the rename. `TURING_DATA_ROOT` and `TURING_CONTACT_EMAIL` take precedence over their legacy `HYPERRESEARCH_*` equivalents.

Evaluate light on usefulness, factual errors, time and cost before proposing more orchestration. Reports can be handed to separate experiment tools; Turing does not run autoresearch loops itself.
