...
| Claim-support audit and re-audit | Tested proof/coverage/finding contracts; not yet in the runner pipeline | `tests/audit.test.ts`, `tests/audit_test.py`: raw-text quote checks, stale hashes, missing reads, revoked local evidence, false numerical direction, fabricated quotes and missing bindings. Async SDK validation is awaited. Prose-block assessment is not sentence-level entailment or proof of truth; scheduled model assessment, corrections, re-audit and comparative evaluations pending |
